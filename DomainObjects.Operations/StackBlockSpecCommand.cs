...
namespace DomainObjects.Operations
{
        ...
        where TOperationEvent : IOperationEvent
    {
        ...
        {
            if (func != null)
                return new Command<TState, TOperationEvent>(Tag, state, stackEvents, func);
            else if (action != null)
                return new Command<TState, TOperationEvent>(Tag, state, stackEvents, action);
            else if (funcWithResult != null)
                return new Command<TState, TOperationEvent>(Tag, state, stackEvents, funcWithResult);
            else if (funcAsync != null)
                return new Command<TState, TOperationEvent>(Tag, state, stackEvents, funcAsync);
            else if (actionAsync != null)
                return new Command<TState, TOperationEvent>(Tag, state, stackEvents, actionAsync);
            else
                return new Command<TState, TOperationEvent>(Tag, state, stackEvents, funcWithResultAsync);
        }
    }
        ...
        where TOperationEvent : IOperationEvent
    {
        ...
        public override StackBlockBase<TState, TOperationEvent> CreateBlock(TState state, IStackEvents<TOperationEvent> stackEvents, IEmptyable input)
        {
            ...
            if (func != null)
                return new Command<TState, TOperationEvent, Tin>(Tag, state, stackEvents, typedInput, func);
            else if (action != null)
                return new Command<TState, TOperationEvent, Tin>(Tag, state, stackEvents, typedInput, action);
            else if (funcWithResult != null)
                return new Command<TState, TOperationEvent, Tin>(Tag, state, stackEvents, typedInput, funcWithResult);
            else if (funcAsync != null)
                return new Command<TState, TOperationEvent, Tin>(Tag, state, stackEvents, typedInput, funcAsync);
            else if (actionAsync != null)
                return new Command<TState, TOperationEvent, Tin>(Tag, state, stackEvents, typedInput, actionAsync);
            else
                return new Command<TState, TOperationEvent, Tin>(Tag, state, stackEvents, typedInput, funcWithResultAsync);
        }
    }
}
