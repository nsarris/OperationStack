...
namespace DomainObjects.Operations
{
    ...
    {
        internal class CommandBlock : StackBlockBase, ICommand<TInput, TState>
        {
            private readonly ResultVoidDispatcher<TState> resultDispatcher = new ResultVoidDispatcher<TState>();

            protected CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents)
                : base(tag, input, state, stackEvents)
            {

            }

            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, Func<ICommand<TInput, TState>, BlockResultVoid> func)
                : base(tag, input, state, stackEvents)
            {
                executor = () => func(this);
            }
            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, Action<ICommand<TInput, TState>> action)
                : base(tag, input, state, stackEvents)
            {
                executor = () => { action(this); return resultDispatcher.Return(); };
            }

            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, Func<IOperationBlock<TInput, TState>, ICommandResult> func)
                : base(tag, input, state, stackEvents)
            {
            ...
            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, Func<ICommand<TInput, TState>, Task<BlockResultVoid>> func)
                : base(tag, input, state, stackEvents)
            {
                executorAsync = async () => await func(this).ConfigureAwait(false);
            }

            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, Func<ICommand<TInput, TState>, Task> actionAsync)
                : base(tag, input, state, stackEvents)
            {
                executorAsync = async () => { await actionAsync(this).ConfigureAwait(false); return resultDispatcher.Return(); };
            }

            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, Func<IOperationBlock<TInput, TState>, Task<ICommandResult>> func)
                : base(tag, input, state, stackEvents)
            {
                executorAsync = async () => { var r = await func(this).ConfigureAwait(false); this.Append(r); return resultDispatcher.Return(); };
            }

            internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, ICommandOperation commandOperation)
                : base(tag, input, state, stackEvents)
            {
                if (commandOperation.SupportsAsync && commandOperation.PreferAsync)
                    ...
                    {
                        ICommandResult r;
                        if (commandOperation is ICommandOperationWithState<TState> operationWithState)
            ...
            //internal CommandBlock(string tag, TInput input, TState state, IStackEvents stackEvents, ICommandOperationWithState<TState> commandOperation)
            //    : base(tag, input, state, stackEvents)
            //{
            //    if (commandOperation.SupportsAsync && commandOperation.PreferAsync)
